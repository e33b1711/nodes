  ...
  for (int i = 0; i < num_rollos; i++) {
    if (rollos[i].value != rollos[i].old_value) {
      ...
      switch (rollos[i].value) {
        case 0:
          write_output(rollos[i].l_up, 1, false);
          write_output(rollos[i].l_down, 0, false);
          rollos[i].stop_time = millis() + rollos[i].duration;
          rollos[i].stop_pending = true;
          break;
        case 100:
          write_output(rollos[i].l_up, rollos[i].is_rollo, false);
          write_output(rollos[i].l_down, 1, false);
          rollos[i].stop_time = millis() + rollos[i].duration;
          rollos[i].stop_pending = true;
          break;
        default:
          write_output(rollos[i].l_up, 0, false);
          write_output(rollos[i].l_down, 0, false);
...
bool write_rollo(String name, int value, bool silent) {
  for (int i = 0; i < num_rollos; i++) {
    if (rollos[i].name == name) {
      ...
      if (!silent) send_state(name, rollos[i].value);
      return true;
    }
  }
  return false;
